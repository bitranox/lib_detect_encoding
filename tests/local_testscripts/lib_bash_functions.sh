...
function cleanup() {
  ...
  rm -f  /usr/local/bin/lib_detect_encoding
  cd "${save_dir}" || exit
...
function test_commandline_interface_venv() {
  ...
  clr_green "issuing command : /usr/local/bin/lib_detect_encoding --version"
  if ! "/usr/local/bin/lib_detect_encoding" --version; then
...
function test_commandline_interface_venv_old() {
  ...
  clr_green "issuing command : $HOME/venv/local/bin/lib_detect_encoding --version"
  if ! "$HOME/venv/local/bin/lib_detect_encoding" --version; then
    my_banner_warning "test commandline interface on virtual environment ERROR"
    beep
    sleep "${sleeptime_on_error}"
    return 1
